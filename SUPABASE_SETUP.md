...
crashes the server at boot on purpose (see `elms/server/src/config/env.js`).
...
You can also paste `elms/server/schema.sql` into the Supabase **SQL Editor** if you
...
- **Uploads stay on disk** (`elms/server/uploads/`), not in Supabase Storage. Files are
...
- **Tests** (`npm test` in `elms/server/`) hit the real `DATABASE_URL`. Point them at a
