...
export default function Layout({ children, links = [] }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 lg:flex">
      {/* Sidebar — desktop */}
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col bg-accent-600 p-4 lg:flex">
        <Link to="/" className="mb-6 flex items-center gap-2 px-2 py-2 text-white">
          <span className="grid h-9 w-9 place-items-center rounded-lg bg-white/20 text-sm font-black">
            E
          </span>
          <span className="text-xl font-bold tracking-tight">ELMS</span>
        </Link>
        {nav}
      ...
      {open && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 flex w-64 flex-col bg-accent-600 p-4">
            <span className="mb-6 px-2 py-2 text-xl font-bold text-white">ELMS</span>
            {nav}
          </aside>
        </div>
      )}
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 sm:px-6">
            ...
            <span className="truncate text-sm text-slate-500 lg:text-base">
            ...
            <div className="flex shrink-0 items-center gap-3">
              ...
              <span className="grid h-9 w-9 place-items-center rounded-full bg-accent-50 text-sm font-bold uppercase text-accent-700">
